    ...
    def apply(self, ctx: EffectContext) -> Any:
        ...
        if pre_spd is not None and post_spd is not None and anchor is not None and src_dur is not None:
            ...
            if pre_out > 0.05 and post_out > 0.05 and pre_src > 0.0 and post_src > 0.0:
                pre_v = pre_src / pre_out
                post_v = post_src / post_out
                src_eps = max(0.0, src_dur - 1.0 / max(1, int(ctx.fps)))

                def aligned_time_map(t: Any) -> Any:
                    # MoviePy passes scalars for video frames but numpy arrays for
                    # audio chunks; the map must be vectorized.
                    ...
                    src = np.where(
                        ...
                        np.where(
                            tt <= pre_out + slow_out,
                            pre_src + (tt - pre_out) * float(slowmo_fac),
                            pre_src + slow_src + (tt - pre_out - slow_out) * post_v,
                        ),
                    )
                    src = np.clip(src, 0.0, src_eps)
                    if np.ndim(t) == 0:
                        return float(src)
                    return src
